...
const App = () => {
    ...
    if (duplicate) {
      ...
      contactsService
        ...
        .then((returnedContact) => {
          ...
          setNewName('')
          setNewNumber('')
          setNotification([`Added ${returnedContact.name}`, 0])
          setTimeout(() => {
  ...
  const handleDeletion = (id) => {
    if (
      ...
      contactsService.deleteById(id).then(() => {
  ...
  return (
    <div>
      ...
      <h2>Numbers</h2>
      <Persons
        searched={persons.filter((person) =>
          searched.length === 0 && newSearch === ''
            ? persons
            : searched.includes(person)
        )}
...
const Notification = ({ message }) => {
  if (message === null) {
    ...
    return <div className='success_message'>{message[0]}</div>
  }
  return <div className='error_message'>{message[0]}</div>
}
